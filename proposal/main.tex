...
\begin{itemize}
    \item \textbf{Member 1:} Rudra Dudhat, B24DS506, rudramd@iitbhilai.ac.in. System design and implementation of all three key value cache backends, the shared attention module, and the benchmarking and plotting scripts.
    \item \textbf{Member 2:} Vatsal Yadav, B24DS036, vatsal@iitbhilai.ac.in. Verification of the implementation's correctness, design of the evaluation metrics, and independent validation of the reported results against the source papers' claims.
    \item \textbf{Member 3:} Mohak Arya, 12341420, mohak@iitbhilai.ac.in. Literature review, ideation of the project's research angle, and preparation of the written deliverables, including this proposal.
\end{itemize}
...
This is not just restating the two papers. We built one shared implementation that isolates both claims side by side, under our own measurement, and we can defend every design choice in it.
...
\begin{itemize}
    ...
    \item Deliverables: this GitHub repository (code, results, and this proposal), and a viva ready explanation of why non-contiguous memory costs attention speed, and why vAttention's fix, decoupling virtual contiguity from physical allocation, recovers it.
\end{itemize}
...
Claude (Anthropic, Claude Code CLI) was used for this project. It helped search and summarize the PagedAttention, vAttention, and Sarathi-Serve literature so the problem statement is grounded in cited, verifiable claims. It also helped draft the initial Python implementation of the three KV-cache backends and the benchmark and plotting scripts, which we then ran, inspected, and checked against the papers' claims ourselves, and helped draft this proposal from our own decisions. Every number in this proposal comes from code we actually ran (\texttt{src/benchmark.py} in the repository), not from the papers and not from generated text we did not verify.
